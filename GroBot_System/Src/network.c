...
extern char* SSID = "WIFIVE";
extern char* PASSWORD = "Namluhcs1";
extern int SECURITY = SL_SEC_TYPE_WPA;
//*****************************************************************************
...
//                 	*Initialize variables for network settings
//				   	*Configure to default state
...
int Network_Init() {
    ...
    UART_PRINT("Initializing network settings \n\r");
...
//                 	*Connect to Access Point using
//					 SSID_NAME and SECURITY_KEY declared in Network.h
...
int Connect_To_AP()
{
	if(initialized) {
		...
		UART_PRINT("Network settings have not been initialized.\n\r");
		return -1;
	}
}

int Connect_To_Server() {
	if(initialized && ap_connected) {
		long lRetVal = -1;
		...
		UART_PRINT("Connection to Access Point has not been established. \n\r");
		return -1;
	}
}
...
static long ConfigureSimpleLinkToDefaultState()
{
    ...
    if (ROLE_STA != lMode)
    {
    	UART_PRINT("Switching device to station mode\r\n");
        if (ROLE_AP == lMode)
        {
...
static long WlanConnect()
{
    ...
    lRetVal = sl_WlanConnect((signed char *)SSID,
                           strlen((const char *)SSID), 0, &secParams, 0);
    ASSERT_ON_ERROR(lRetVal);
